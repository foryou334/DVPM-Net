...
class CoordAtt(nn.Module):
    def __init__(self, inp, oup, reduction=32):
        ...
        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))  # 沿宽度方向压缩
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))  # 沿高度方向压缩
...
class HighResolutionNet(nn.Module):
    def __init__(self, base_channel: int = 32, num_joints: int = 14):
        ...
        # 引入坐标注意力机制
        def make_ca(channels): return CoordAtt(channels, channels)

        # Stem部分
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.ca1 = make_ca(64)  # 添加CA
        self.relu = nn.ReLU(inplace=False)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.ca2 = make_ca(64)  # 添加CA

        # Layer1: Bottleneck结构
        self.layer1 = self._make_layer(Bottleneck, 64, base_channel * 4, blocks=4)
        self.ca3 = make_ca(base_channel * 4 * Bottleneck.expansion)
        ...
        self.transition2 = nn.ModuleList([
            nn.Sequential(make_ca(base_channel)),
            nn.Sequential(make_ca(base_channel * 2)),
            nn.Sequential(
        ...
        self.transition3 = nn.ModuleList([
            nn.Sequential(make_ca(base_channel)),
            nn.Sequential(make_ca(base_channel * 2)),
            nn.Sequential(make_ca(base_channel * 4)),
            nn.Sequential(
    ...
    def forward(self, x):
        ...
        x_list = [self.ca_stage2[i](x_list[i]) for i in range(len(x_list))]
        ...
        x_list = [self.ca_stage3[i](x_list[i]) for i in range(len(x_list))]
        ...
        x_list = [self.ca_stage4[i](x_list[i]) for i in range(len(x_list))]
