...
def main(args):
    ...
    data_transform = {
        "test": transforms.Compose([
            # transforms.AffineTransform(fixed_size=args.resize_hw),
            transforms.ToTensor(),
    ...
    val_dataset = CocoKeypoint(data_root, "test", transforms=data_transform["test"], det_json_path=None)
    ...
    checkpoint = torch.load(weights_path, map_location='cuda:2')
    ...
    with torch.no_grad():
        for images, targets in tqdm(val_dataset_loader, desc="validation..."):
            ...
            outputs = transforms.get_final_preds(outputs, None, post_processing=True)
    ...
    """
    key_metric.synchronize_results()
    print("✅ Done! Prediction results saved to: key_results.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resize-hw', default=[960,1280], help="resize for predict")
    # 是否开启图像翻转
    parser.add_argument('--flip', type=bool, default=True, help='whether using flipped images')

    # # 数据集的根目录
    # parser.add_argument('--data-path', default='/data/SCU', help='dataset root')

        # 数据集的根目录
    parser.add_argument('--data-path', default='/data/chengfei', help='dataset root')

    # 训练好的权重文件
    parser.add_argument('--weights-path', default='./chengfei_save_weights/model-209.pth', type=str, help='training weights')

    # batch size
    parser.add_argument('--batch-size', default=1, type=int, metavar='N',
                        help='batch size when validation.')
    # 类别索引和类别名称对应关系
    parser.add_argument('--label-json-path', type=str, default="target_keypoints.json")
