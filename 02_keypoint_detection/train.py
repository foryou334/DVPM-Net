...
def create_model(num_joints, load_pretrain_weights=False):
    ...
    if load_pretrain_weights:
        ...
        weights_dict = torch.load("./chengfei_save_weights_1/model-97.pth", map_location='cuda:2')
...
def main(args):
    ...
    data_transform = {
        "train": transforms.Compose([
            ...
            # transforms.AffineTransform(scale=(0.65, 1.35), rotation=(-5, 5), fixed_size=fixed_size),
            transforms.RandomHorizontalFlip(0, person_kps_info["flip_pairs"]),
    ...
    if args.resume != None:
        checkpoint = torch.load(args.resume, map_location='cuda:2')
...
##############################################################################################
    ...
    if args.resume and os.path.exists(args.resume):
        ...
        if "best_map" in checkpoint:
            ...
            print(f"=> Resumed best mAP: {best_map:.4f}")
##############################################################################################
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # 判断是否为最优模型
        is_best = coco_info[1] > best_map
        print(f"save best : {is_best}")
        if is_best:
            best_map = coco_info[1]
            save_files["best_map"] = best_map
            torch.save(save_files, os.path.join(args.output_dir, "best.pth"))
...
if __name__ == "__main__":
    ...
    # # 训练数据集的根目录(coco2017)
    # parser.add_argument('--data-path', default='./data/SCU', help='dataset')
    # # SCU数据集关键点信息
    # parser.add_argument('--keypoints-path', default="./target_keypoints.json", type=str,
    #                     help='target_keypoints.json path')
    
    # 训练数据集的根目录(coco2017)
    parser.add_argument('--data-path', default='./data/chengfei', help='dataset')
    # SCU数据集关键点信息
    parser.add_argument('--keypoints-path', default="./chengfei_target_keypoints.json", type=str,
                        help='chengfei_target_keypoints.json path')


    # 原项目提供的验证集person检测信息，如果要使用GT信息，直接将该参数置为None，建议设置成None
    parser.add_argument('--person-det', type=str, default=None)
    parser.add_argument('--fixed-size', default=[960,1280], nargs='+', type=int, help='input size')
    # keypoints点数
    parser.add_argument('--num-joints', default=14, type=int, help='num_joints')
    # 文件保存地址
    parser.add_argument('--output-dir', default='./chengfei_save_weights_2', help='path where to save')
    ...
    parser.add_argument('--lr-steps', default=[30, 50], nargs='+', type=int, help='decrease lr every step-size epochs')
    # 针对torch.optim.lr_scheduler.MultiStepLR的参数
